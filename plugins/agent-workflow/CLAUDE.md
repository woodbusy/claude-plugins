...
**Skills** are user-invocable slash commands (SKILL.md files) that handle orchestration - sequencing agents, managing parallel execution, and coordinating multi-round review cycles. The `workflow-start` skill orchestrates the full pipeline. The `workflow-resume` skill provides a re-entry point for resuming in-progress workflows. The `team-review` skill handles review-specific orchestration: it classifies the diff into change categories, selects an appropriate reviewer team (always-on tech-lead and security plus application/infra-platform/dev-platform specialists when their domains are touched), and manages parallel launch and round 2 re-runs.
